...
public:
	...
	*/
	...
	/**
		@brief
		\~English	Enable a snapshot cache for a handle so that UpdateHandleToMoveToFrame does not
					always have to replay from the beginning (or from the current frame) to reach
					the requested frame. Hidden shadow instances of the same effect are kept running
					in the background at regular frame intervals; a seek resumes from the nearest one.
		\~Japanese	指定したハンドルにスナップショットキャッシュを有効にする。UpdateHandleToMoveToFrame
					が毎回先頭(または現在位置)からの再計算を行わずに済むよう、同じエフェクトの非表示の
					シャドウインスタンスを一定フレーム間隔でバックグラウンド再生しておき、シーク時は
					一番近いものから再開する。
		@param	handle
		\~English	a handle.
		\~Japanese	ハンドル
		@param	checkpointInterval
		\~English	frame interval (1 is 1/60 seconds) between kept checkpoints.
		\~Japanese	チェックポイントを保持するフレーム間隔(60fps基準)。
		@param	maxCheckpoints
		\~English	the maximum number of checkpoints kept alive at once (bounds the extra CPU/memory cost).
		\~Japanese	同時に保持するチェックポイントの最大数(追加のCPU・メモリコストの上限)。
		@note
		\~English	Not supported for effects using GPU particles; the call is ignored for such a handle
					and UpdateHandleToMoveToFrame falls back to the original behavior.
		\~Japanese	GPUパーティクルを使用するエフェクトには対応していない。その場合呼び出しは無視され、
					UpdateHandleToMoveToFrameは従来通りの動作にフォールバックする。
	*/
	virtual bool EnableSeekCache(Handle handle, float checkpointInterval = 30.0f, int32_t maxCheckpoints = 8) = 0;

	/**
		@brief
		\~English	Disable a snapshot cache enabled by EnableSeekCache and discard its checkpoints.
		\~Japanese	EnableSeekCacheで有効にしたスナップショットキャッシュを無効化し、チェックポイントを破棄する。
	*/
	virtual void DisableSeekCache(Handle handle) = 0;
