 ...
 *      Include files
 ***************************************************************************/
#include <External/CRI/Include/cri_le_xpt.h>
#include <External/CRI/Include/cri_le_atom.h>
#include <External/CRI/Include/cri_le_file_system.h>
...
#define CRIATOMEX_VER_NUM		"2.30.5"
#define CRIATOMEX_VER_MAJOR		(2)
#define CRIATOMEX_VER_MINOR		(30)
#define CRIATOMEX_VER_PATCH		(5)
 ...
 * \ingroup ATOMEXLIB_ACF
 * \par 説明:
 * バスの出力オプションのビットマスクです。<br>
 * CriAtomExAcfDspBusInfo::output_options の値に使用します。<br>
 * ビットが有効な場合、ツールにて配信制御オプションが設定されているバスとなります。<br>
 * \sa CriAtomExAcfDspBusInfo
 */
#define	CRIATOMEXACF_DSP_BUS_OUTPUT_OPTION_RESTRICTED 	(0b00000001)
 ...
 * \ingroup ATOMEXLIB_ACF
 * \par 説明:
 * ツールで設定したバスの出力タイプです。<br>
 * パッド出力や個人出力は、プラットフォーム機能に依存します。<br>
 * 対応状況については、各種プラットフォーム向けのマニュアルを参照してください。<br>
 * \sa CriAtomExAcfDspBusInfo
 */
typedef enum CriAtomExAcfDspBusOutputTypeTag {
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_NONE = 0,				/*JP< ルーティングバス */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_MAIN,					/*JP< メインスピーカー出力 */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_MAIN_PASSTHROUGH,		/*JP< メインパススルー出力 */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_PAD_HAPTIC,			/*JP< パッド音声振動出力 */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_PAD_SPEAKER,			/*JP< パッドスピーカー出力 */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_PERSONAL,				/*JP< 個人出力 */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_PERSONAL_PASSTHROUGH,	/*JP< 個人出力パススルー */
	CRIATOMEXACF_DSP_BUS_OUTPUT_TYPE_ENUM_SIZE_IS_4BYTES = 0x7FFFFFFF
} CriAtomExAcfDspBusOutputType;
...
typedef struct CriAtomExAcfDspBusInfoTag {
	...
	CriAtomSpeakerMapping speaker_mapping;				/*JP< スピーカーマッピング			*/
	CriAtomExAcfDspBusOutputType output_type;			/*JP< 出力タイプ					*/
	CriUint8 output_options;							/*JP< 出力オプション				*/
...
typedef struct CriAtomExAcbInfoTag {
	...
	CriUint32 version;								/*JP< ACBバージョン				*/
	const CriChar8* user_data;						/*JP< ユーザーデータ			*/
	const CriChar8* language;						/*JP< 出力言語					*/
	CriAtomExCharacterEncoding character_encoding;	/*JP< 文字コード				*/
...
typedef struct CriAtomExCuePos3dInfoTag {
	...
	CriFloat32 cone_outside_volume;		/*JP< コーン外部音量				*/
...
typedef enum CriAtomExSpeakerIdTag {
	...
	CRIATOMEX_SPEAKER_TOP_FRONT_LEFT = 8,		/*JP<トップフロントレフトスピーカー		*/
	CRIATOMEX_SPEAKER_TOP_FRONT_RIGHT = 9,		/*JP<トップフロントライトスピーカー		*/
	CRIATOMEX_SPEAKER_TOP_BACK_LEFT = 10,		/*JP<トップバックレフトスピーカー		*/
	CRIATOMEX_SPEAKER_TOP_BACK_RIGHT = 11,		/*JP<トップバックライトスピーカー		*/
	CRIATOMEX_SPEAKER_BOTTOM_FRONT_LEFT = 12,	/*JP<ボトムフロントレフトスピーカー		*/
	CRIATOMEX_SPEAKER_BOTTOM_FRONT_RIGHT = 13,	/*JP<ボトムフロントライトスピーカー		*/
	CRIATOMEX_SPEAKER_BOTTOM_BACK_LEFT = 14,	/*JP<ボトムバックレフトスピーカー		*/
	CRIATOMEX_SPEAKER_BOTTOM_BACK_RIGHT = 15,	/*JP<ボトムバックライトスピーカー		*/
...
typedef struct CriAtomExCueLinkInfoTag {
	...
	CriAtomExSourceInfo base_cue;		/*JP< リンク元キュー。メンバ変数'type'の値は必ずCRIATOMEX_SOURCE_TYPE_CUE_IDとなる。		*/
	CriAtomExPlaybackId target_id;		/*JP< リンク先再生ID		*/
	CriAtomExSourceInfo target_cue;		/*JP< リンク先キュー。メンバ変数'type'の値は必ずCRIATOMEX_SOURCE_TYPE_CUE_IDとなる。		*/
 ...
 * 		：
 * \endcode
 * ミキサーVersion2のACFを使用する場合、追加で以下の動作を行います。<br>
 * - もとのチャンネル数、サンプリングレート、機種固有設定などを上書きします。<br>
 * - アタッチ時に再生中の音声をすべて停止します<br>
 * \par 備考:
 ...
 * DSPバス設定をデタッチします。<br>
 * ミキサーVersion2のACFを使用する場合、追加で以下の動作を行います。<br>
 * - ASRの状態はアタッチ前に戻します（上書きされたパラメーターはもとに戻ります）<br>
 * ::criAtomEx_SetUserAllocator マクロを使用してアロケーターを登録済みの場合、
 ...
 * 本関数を使用することで、対象のミキサーを使用するプレーヤーで再生される全てのHCA-MXボイス
 ...
 * <br>
 * ミキサーIDは再生開始前に設定する必要があります。<br>
 * 既に再生が開始された音声に対し、後からミキサーIDを変更することはできません。<br>
 ...
 * ::criAtomExHcaMx_SetAsrRackId 関数を使用して、HCA-MXミキサー自体の出力先ASRラックIDを設定してください。<br>
 * ::criAtomExHcaMx_SetAsrRackId 関数を使用して、HCA-MXミキサー自体の出力先ASRラックIDを設定してください。<br>
 ...
 * 直して再生を開始するためです。<br>
 * 設定した値は::criAtomExPlayer_SetStartTimeMicroによる設定を上書きします。<br>
 * 本パラメーターは ::criAtomExPlayer_ResetParameters 関数にてクリアされます。
 * \attention
