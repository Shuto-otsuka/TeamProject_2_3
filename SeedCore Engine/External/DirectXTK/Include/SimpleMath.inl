    ...
    const Vector3* cameraForward) noexcept
{
    ...
    XMVECTOR Z = XMVectorSubtract(C, O);
    ...
    const Vector3* objectForward) noexcept
{
    ...
    const XMVECTOR O = XMLoadFloat3(&object);
    const XMVECTOR C = XMLoadFloat3(&cameraPosition);
    XMVECTOR faceDir = XMVectorSubtract(C, O);
    ...
    if (XMVector3Greater(dot, s_minAngle))
    {
        if (objectForward)
        {
            ...
            if (XMVector3Greater(dot, s_minAngle))
            {
                dot = XMVectorAbs(XMVector3Dot(Y, g_XMIdentityR2));
                Z = (XMVector3Greater(dot, s_minAngle)) ? g_XMIdentityR0 : g_XMIdentityR2;
            }
        }
        else
        {
...
{
    using namespace DirectX;
    XMStoreFloat4(reinterpret_cast<XMFLOAT4*>(this), PackedVector::XMLoadColor(&Packed));
}

inline Color::Color(const DirectX::PackedVector::XMUBYTEN4& Packed) noexcept
{
    using namespace DirectX;
    XMStoreFloat4(reinterpret_cast<XMFLOAT4*>(this), PackedVector::XMLoadUByteN4(&Packed));
}
...
{
    using namespace DirectX;
    const XMVECTOR c1 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(this));
    const XMVECTOR c2 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&c));
...
{
    using namespace DirectX;
    const XMVECTOR c = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(this));
...
{
    using namespace DirectX;
    const XMVECTOR c1 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&C1));
    const XMVECTOR c2 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&C2));
    Color R;
...
{
    using namespace DirectX;
    const XMVECTOR c = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&C));
    Color R;
...
{
    using namespace DirectX;
    const XMVECTOR c1 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&C));
    Color R;
...
{
    using namespace DirectX;
    const XMVECTOR clr = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(this));
...
inline void Color::Premultiply() noexcept
{
    ...
    XMStoreFloat4(reinterpret_cast<XMFLOAT4*>(this), XMVectorMultiply(c, a));
}
...
inline void Color::Premultiply(Color& result) const noexcept
{
    ...
    XMVECTOR a = XMVectorSplatW(c);
    a = XMVectorSelect(g_XMIdentityR3, a, g_XMSelect1110);
    XMStoreFloat4(reinterpret_cast<XMFLOAT4*>(&result), XMVectorMultiply(c, a));
}
...
{
    using namespace DirectX;
    const XMVECTOR C0 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&c1));
    const XMVECTOR C1 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&c2));
