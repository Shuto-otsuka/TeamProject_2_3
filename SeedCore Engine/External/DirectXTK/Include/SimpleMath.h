...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        //------------------------------------------------------------------------------
        // Color
        struct DIRECTX_TOOLKIT_API Color
        {
            union
            {
            ...
            Color() noexcept : r(0), g(0), b(0), a(1.f) {}
            constexpr Color(float _r, float _g, float _b) noexcept : r(_r), g(_g), b(_b), a(1.f) {}
            constexpr Color(float _r, float _g, float _b, float _a) noexcept : r(_r), g(_g), b(_b), a(_a) {}
            explicit Color(const Vector3& clr) noexcept : r(clr.x), g(clr.y), b(clr.z), a(1.f) {}
            explicit Color(const Vector4& clr) noexcept : r(clr.x), g(clr.y), b(clr.z), a(clr.w) {}
            explicit Color(_In_reads_(4) const float *pArray) noexcept : r(pArray[0]), g(pArray[1]), b(pArray[2]), a(pArray[3]) {}
            Color(FXMVECTOR V) noexcept { XMStoreFloat4(reinterpret_cast<XMFLOAT4*>(this), V); }
            Color(const XMFLOAT4& c) noexcept : r(c.x), g(c.y), b(c.z), a(c.w) {}
            explicit Color(const XMVECTORF32& F) noexcept : r(F.f[0]), g(F.f[1]), b(F.f[2]), a(F.f[3]) {}
            ...
            operator XMVECTOR() const noexcept { return XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(this)); }
            ...
            // Assignment operators
            Color& operator= (const XMVECTORF32& F) noexcept { r = F.f[0]; g = F.f[1]; b = F.f[2]; a = F.f[3]; return *this; }
            ...
            void R(float _r) noexcept { r = _r; }

            float G() const noexcept { return g; }
            void G(float _g) noexcept { g = _g; }

            float B() const noexcept { return b; }
            void B(float _b) noexcept { b = _b; }

            float A() const noexcept { return a; }
            void A(float _a) noexcept { a = _a; }
            ...
            Vector4 ToVector4() const noexcept { return Vector4(r, g, b, a); }
