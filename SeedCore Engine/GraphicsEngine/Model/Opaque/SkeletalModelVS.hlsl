...
ModelVSOutput main(uint vertex_id : SV_VertexID, uint instance_id : SV_InstanceID)
{
	...
	ModelVSOutput output = (ModelVSOutput)0;
	output.position = float4(2.0, 2.0, 2.0, 1.0);

	if (instance.skining_.skin_index_ == 0xFFFFFFFF || triangle_index >= meshlet.triangle_count_)
	{
		return output;
	}
	...
	output.meshlet_index = visible.meshlet_index_;
	output.triangle_in_meshlet_index = triangle_index;
